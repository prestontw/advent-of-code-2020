...
type Direction = Cardinals * int
exception ParseException of string

let parseDirection (raw: string) =
    let heading =
        match raw.[0] with
        | 'U' -> Up
        | 'R' -> Right
        | 'L' -> Left
        | 'D' -> Down
        | a -> raise (ParseException(sprintf "Case not covered: %c" a))

    let length = int raw.[1..]
    heading, length

let parseLine (line: string) =
    line.Split(',') |> Seq.map parseDirection
...
let move location direction =
    ...
    let times = snd direction

    let newLocations =
        Seq.fold (fun (locations: Map<_, _>) (time: int) ->
            let point =
                fst unitDirection * time, snd unitDirection * time

            if locations.ContainsKey point then
                locations
            else
                locations.Add
                    ((fst location.EndingPoint + fst point, snd location.EndingPoint + snd point), location.Time + time))
            (location.HasBeen) ([ 1 .. times ])

    let newEndingPoint =
        (fst location.EndingPoint
         + fst unitDirection
         * times,
         snd location.EndingPoint
         + snd unitDirection
         * times)

    { HasBeen = newLocations
      EndingPoint = newEndingPoint
      Time = location.Time + times }
...
let closest set1 set2 =
    ...
    let ordered =
        common
        |> Set.toArray
        |> Array.sortBy manhattanDistance
...
let part1 (input: string) =
    ...
    let [ l1; l2 ] =
        directions |> Seq.map moveAll |> Seq.toList

    closest (l1.HasBeen |> keysToSet |> Set.remove (0, 0)) (l2.HasBeen |> keysToSet |> Set.remove (0, 0))
    |> manhattanDistance
...
let part2 (input: string) =
    ...
    let s1 = l1.HasBeen.Remove(0, 0)
    let s2 = l2.HasBeen.Remove(0, 0)

    let commonLocations =
        Set.intersect (s1 |> keysToSet) (s2 |> keysToSet)
